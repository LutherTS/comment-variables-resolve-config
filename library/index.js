 ...
 * @returns The JSON resolved config data to be written at an expected `.json` path. It can be consumed by any language that can parse JSON, which is virtually all modern languages, so that Comment Variables can act as the single source of truth for text variables beyond JavaScript and TypeScript.
 */
 ...
 * @param {Record<string, unknown>} resolvedConfigData The resolved config data as obtained from `makeResolvedConfigData`.
 * @returns The MJS resolved config data to be written at an expected `.mjs` path. Its format make it possible to be consumed with literal type safety in both JavaScript and TypeScript.
 */
