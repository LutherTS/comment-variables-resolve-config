...
export const resolvedConfigData = {
  "jsDoc": {
    ...
    "params": {
      ...
      "isVariationData": {
        "value": "A boolean that, when `false` or `undefined`, decides to crop out the initial variant segments of composed variables exclusives keys when addressing variation data. (Originally known as `isVariationData`, the argument remains since its logic is already implemented, even though a use case for core data as yet to be found.)",
    ...
    "returns": {
      ...
      "resolveVariationData": {
        "value": "With a `{success: true}` object, the given variation's own information to be consumed by the Comment Variables CLI and the Comment Variables VS Code extension, namingly its own `originalFlattenedConfigData`, its own `aliases_flattenedKeys`, its own `flattenedConfigData`, its own `reversedFlattenedConfigData`, and its own `configDataResultsData`.",
